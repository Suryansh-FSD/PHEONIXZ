...
import { navItemsConfig } from "@/config/navigation";
...
export const AppShell: React.FC = () => {
  ...
  // Load state via API Service Abstraction
  useEffect(() => {
    async function loadData() {
      const header = await fetchSystemHeaderState();
      const stats = await fetchStatStripData();
      const feedRes = await fetchAgentFeed();
      const decs = await fetchDecisionLogItems();
      const act = await fetchActivityData();
      const thrs = await fetchCompetitiveThreads();
  ...
  const handleTriggerScan = () => {
    ...
    setTimeout(() => {
      if (headerState) {
        setHeaderState((prev) => prev ? ({ ...prev, lastScan: "Just now" }) : null);
      }
      if (statStrip) {
        setStatStrip((prev) => prev ? ({
          ...prev,
          observed: prev.observed + 4,
          rejected: prev.rejected + 3,
